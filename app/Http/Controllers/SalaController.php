...
class SalaController extends Controller
{
    ...
    public function crearReserva(Request $request, $id)
    {
        $validatedData = $request->validate([
            ...
            'tipo_reserva' => 'required|in:privada,concierto', // Validar tipo de reserva
            'precio_entrada' => 'nullable|required_if:tipo_reserva,concierto|numeric|min:0', // Precio solo para conciertos
        ]);
        ...
        // Si es un concierto, crea el evento y la entrada
        if ($validatedData['tipo_reserva'] === 'concierto') {
            $evento = Evento::create([
                'nombre_evento' => 'Concierto - ' . $validatedData['descripcion'],
                'descripcion' => $validatedData['descripcion'],
                'fecha_evento' => $validatedData['fecha_reserva'],
                'hora_inicio' => '20:00:00', // Hora predeterminada
                'hora_final' => '23:00:00', // Hora predeterminada
                'sala_id' => $id, // Asocia el evento a la sala reservada
            ]);

            // Crear una entrada para el concierto
            Entrada::create([
                'evento_id' => $evento->id,
                'tipo' => 'normal',
